...
function ProjectDetail(props: { project: Project; onBack: () => void; onProjectUpdated: (p: Project) => void }) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <button onClick={onBack} className="rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm">
          ← Zur Übersicht
        </button>
  ...
  return (
    <button
      ...
      className={
        ...
        (props.active ? "bg-blue-600 text-white border-blue-600" : "bg-white text-slate-700 border-slate-300 hover:bg-slate-50")
      }
    >
      {props.children}
    </button>
...
function OverviewPanel(props: { project: Project; onProjectUpdated: (p: Project) => void }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <div className="rounded-xl border border-slate-200 bg-white p-4 md:p-6 space-y-4">
          ...
          <div className="grid gap-3">
            <Field label="Name">
              <input className="w-full rounded-xl border border-slate-300 px-3 py-2" value={base.name} onChange={(e) => setBase((s) => ({ ...s, name: e.target.value }))} />
            </Field>
            <Field label="Code">
              <input className="w-full rounded-xl border border-slate-300 px-3 py-2" value={base.code} onChange={(e) => setBase((s) => ({ ...s, code: e.target.value }))} />
            </Field>
            <Field label="Status">
            ...
            <Field label="Notizen">
              <textarea className="w-full rounded-xl border border-slate-300 px-3 py-2" rows={4} value={base.notes} onChange={(e) => setBase((s) => ({ ...s, notes: e.target.value }))} />
            </Field>
      ...
      <div className="flex items-center gap-3">
        <button className="rounded-xl bg-blue-600 px-3 py-2 text-white disabled:opacity-50" onClick={handleSave} disabled={!hasChanges || saving}>
